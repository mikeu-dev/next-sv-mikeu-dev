		...
		for (const file of files) {
			try {
				...
				try {
					...
					throw new Error(
						'Local data files are git-ignored and not available in production builds.'
					);
				} catch (err) {
					results.push({
				...
				const patchedData = iconMap
					? patchIcons(cleanData, iconMap as Map<string, string>)
					: cleanData;
				...
				if (file.type === 'techstack' || file.type === 'journey' || file.type === 'skills') {
					...
					if (patchedData.en) {
						const dataToSave = Array.isArray(patchedData.en)
							? { items: patchedData.en }
							: patchedData.en;
						await db.collection(file.collection).doc('en').set(dataToSave);
					}
					if (patchedData.id) {
						const dataToSave = Array.isArray(patchedData.id)
							? { items: patchedData.id }
							: patchedData.id;
