...
<article class="group relative h-full">
	...
	>
		...
		<div class="relative aspect-video overflow-hidden" style="clip-path: polygon(0 0, 100% 0, 100% 85%, 0 100%);">
			<img
			...
			<div class="absolute top-4 left-4 flex flex-col gap-2">
				{#if isNew}
					<div class="flex items-center gap-1 bg-primary px-3 py-1 font-mono text-[9px] font-black text-primary-foreground tracking-widest uppercase">
						<Sparkles class="size-3" /> [NEW_ENTRY]
					</div>
				{/if}
				<div class="flex items-center gap-1 bg-foreground/90 px-3 py-1 font-mono text-[9px] font-black text-background tracking-widest uppercase">
		...
		<div class="flex flex-1 flex-col p-6">
			...
			<div class="mb-4 flex flex-wrap gap-3">
				{#each post.tags || [] as tag (tag)}
					<span class="flex items-center gap-1 font-mono text-[9px] font-bold text-primary tracking-widest uppercase">
						<Hash class="size-2" /> {tag}
					</span>
				{/each}
			</div>

			<div class="flex items-start justify-between gap-4">
				<h3 class="font-poppins text-xl font-black leading-tight tracking-tighter transition-colors group-hover:text-primary lg:text-2xl">
					{post.title}
				</h3>
			...
			<p class="mt-4 line-clamp-2 font-mono text-xs leading-relaxed text-muted-foreground/80 uppercase">
				{post.description}
			</p>
			...
			<div class="mt-auto flex items-center justify-between border-t border-foreground/10 pt-6">
				{#if post.readingTime}
					<div class="flex items-center gap-2 font-mono text-[10px] font-black text-foreground/50 uppercase">
						<Clock class="size-3" />
						<span>{post.readingTime} MIN_READ</span>
					</div>
				{/if}

				<div class="flex items-center gap-1 font-mono text-[10px] font-black text-primary opacity-0 transition-all duration-300 group-hover:opacity-100">
