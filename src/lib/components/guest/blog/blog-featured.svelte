...
>
	<a href={localizeHref(`/blog/${post.slug}`)} class="flex flex-col md:flex-row">
		<!-- Image Section -->
		<div class="relative h-72 overflow-hidden md:h-auto md:w-1/2"
			...
			/>
			<div class="absolute inset-0 bg-primary/10 opacity-0 transition-opacity group-hover:opacity-100"></div>
			
			<!-- Featured Label -->
			<div class="absolute top-6 left-6">
				<div class="bg-primary px-4 py-1 font-mono text-[10px] font-black tracking-[0.2em] text-primary-foreground uppercase shadow-[4px_4px_0_var(--foreground)]">
					[FEATURED_ENTRY]
				</div>
			</div>
		</div>
		...
		<div class="flex flex-col justify-center p-8 md:w-1/2 md:p-12 lg:p-16">
			...
			<p class="mt-8 line-clamp-3 font-mono text-xs leading-relaxed tracking-tight text-muted-foreground uppercase md:text-sm">
				// {post.description}
			</p>

			<div class="mt-10 flex flex-wrap items-center gap-8 border-t border-foreground/10 pt-8">
				<div class="flex items-center gap-2 font-mono text-[10px] font-black tracking-widest text-foreground/40 uppercase">
			...
			<div class="mt-10">
				...
				>
					{m.blog_card_button()}
					<Icon iconName="BsArrowRight" size={14} class="transition-transform group-hover:translate-x-1" />
				</div>
			</div>
		</div>
	</a>
