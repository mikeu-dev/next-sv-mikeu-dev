...
>
	...
	<div class="container relative mx-auto px-6">
		<div bind:this={shardsContainer} class="shards-wrapper relative flex flex-col items-center">
			...
			></div>
			<div 
				class="origami-bg-shard absolute -bottom-12 -right-12 size-80 bg-foreground/5"
				style="clip-path: polygon(15% 0%, 100% 15%, 85% 85%, 0% 100%);"
			></div>

			<!-- Technical Labels -->
			<div class="mb-12 flex w-full flex-wrap items-center justify-between gap-4 border-b-2 border-foreground pb-6">
				<div class="technical-label flex items-center gap-2 font-mono text-[10px] font-black text-primary uppercase">
					<Hash class="size-3" /> PROTOCOL: CONTACT_INIT_V2
				</div>
				<div class="technical-label flex items-center gap-2 font-mono text-[10px] font-black text-foreground/50 uppercase">
			...
			<div class="contact-content relative z-10 text-center">
				<div class="mb-6 inline-flex items-center gap-2 rounded-full border border-foreground/10 bg-foreground/5 px-4 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest text-foreground/80">
					<Mail class="size-3 text-primary" /> {m.contact_field_email()}
				</div>
				...
				<p class="mx-auto mb-16 max-w-2xl font-mono text-sm leading-relaxed text-muted-foreground uppercase sm:text-base">
					{m.contact_subtitle_first_part()}
					<span class="block mt-2 text-foreground font-bold">
				...
				>
					<!-- Button Hover Effect -->
					<div class="absolute inset-0 bg-foreground opacity-0 transition-opacity group-hover:opacity-10"></div>
			...
			<!-- Technical Footer -->
			<div class="mt-16 w-full pt-6 border-t-2 border-foreground/10">
				<div class="flex justify-between font-mono text-[8px] font-black tracking-[0.2em] text-foreground/40 uppercase">
